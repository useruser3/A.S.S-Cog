...
async def gameselection(self,member,channel):
    print("running")
    ...
    if msg.content.startswith("1"):
        ...
        if msg.content.startswith("1"):#hr 1-100
            ...
            #await timezoneselect(self,member = member,channel = channel) TODO
...
async def timezoneselect(self,member,channel):
    print("running2")
    channel = self.bot.get_channel(481295578530185226)
    server = channel.guild
    await channel.send(f"{member.mention} please pick your timezone from the list below")
    ...
    #timezones
    if msg.content.startswith("hello"):#gmt
